...
O `tnrx` também escolhe a primeira porta livre a partir de 8888 no nó do job e passa ao Jupyter a URL certa (`http://<nó>:<porta>`). Isso substitui o `http://hostname:8888` que a imagem NGC imprime por padrão (texto fixo no `jupyter_notebook_config.py` dela). Ele imprime `🌐 [tnrx] Nó: dl-05 | porta: 8888` antes do Jupyter subir: use esse nome e essa porta na URL. Do seu laptop, essa URL só abre com uma ponte SSH, que o [`tnrx-connect jupyter`](tnrx-connect.md#jupyter-no-nó-de-computação) cria por você. Para isso o `tnrx` gera o token do Jupyter e, **quando o Jupyter começa a responder**, grava `.tnrx/jupyter/<job>.env` (permissão 600, com nó, porta, token e job) na pasta do projeto, que o `tnrx-connect` lê. A pasta `.tnrx/` tem um `.gitignore` próprio que ignora tudo, então o token não vai para um `git add .`. Se o Jupyter demorar mais de 3 minutos para responder, nada é gravado (ajustável com `TNRX_JUPYTER_REGISTER_SECS`). Um `--port` passado na linha de comando é ignorado, porque a flag do `tnrx` vem depois; para começar por outra porta, use `TNRX_JUPYTER_PORT=9000 tnrx uvslurm jupyter lab`.
