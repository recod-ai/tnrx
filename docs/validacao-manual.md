...
| ☑ | `Ctrl-C` chega ao servidor | Dentro do comando, dois `Ctrl-C` em menos de 1 segundo | **Validado (2026-09-21, Headnode):** chega ao `srun` (`interrupt (one more within 1 sec to abort)`), o job é cancelado (`STEP ... CANCELLED`), o ssh e a conexão mestra fecham. O prompt `y/n` do Jupyter **não** aparece: o `srun` intercepta. Falta conferir `squeue -u $USER` vazio depois |
...
| ☑ | `tnrx` desatualizado | Rode sem dar `git pull` no servidor | **Validado (2026-09-21):** sem marcador, não abriu ponte, mostrou a dica e terminou com erro claro. Depois disso ganhou o plano B: usa a URL que o Jupyter imprime (`http://dl-05:8888/lab?token=...`) quando o token está visível. **Falta validar** esse plano B no servidor |
